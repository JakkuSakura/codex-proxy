...
def test_responses_api_sse_sequence(mock_handler_context):
    ...
    with patch.object(handler.gemini_provider.session, 'post', return_value=mock_post_ctx):
        with patch.object(handler.gemini_provider.auth, 'get_access_token', return_value="token"):
            with patch.object(handler.gemini_provider.auth, 'get_project_id', return_value="pid"):
                handler.path = "/v1/responses"
                handler.do_POST()
                
                # Capture all writes to wfile.wfile
                written_data = b"".join([call.args[0] for call in handler.wfile.write.call_args_list]).decode()
