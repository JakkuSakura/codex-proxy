...
def test_gemini_config_and_stream(mock_handler_context):
    ...
    with patch.object(handler.gemini_provider.session, 'post', return_value=mock_post_ctx) as mock_post:
        with patch.object(handler.gemini_provider.auth, 'get_access_token', return_value="fake_token"):
            with patch.object(handler.gemini_provider.auth, 'get_project_id', return_value="fake_pid"):
...
def test_gemini_non_stream_accumulation(mock_handler_context):
    ...
    with patch.object(handler.gemini_provider.session, 'post', return_value=mock_post_ctx):
        with patch.object(handler.gemini_provider.auth, 'get_access_token', return_value="fake_token"):
            with patch.object(handler.gemini_provider.auth, 'get_project_id', return_value="fake_pid"):
                ...
                assert response_data['output'][0]['content'][0]['text'] == "Part 1 Part 2"
